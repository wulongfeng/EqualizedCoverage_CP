...
class FairGNN(nn.Module):
    def __init__(
        self, nfeat, nclass, sim_coeff=0.6, n_order=10, subgraph_size=30, acc=0.69, epoch=2000
    ):
        ...
        self.estimator = GCN(nfeat, nclass, dropout)
    ...
    def fair_metric_direct(self, pred, labels, sens):
        ...
        #print("parity:{}, equality:{}".format(parity, equality))
    ...
    def optimize(self, g, x, labels, idx_train, sens, idx_sens_train, edge_index):
        ...
        #print("y({}):{}, labels({}):{}".format(y[idx_train].shape, y[idx_train], labels[idx_train].shape, labels[idx_train]))
    ...
    def fit(
        ...
        #self.edge_index = torch.tensor(g.to_dense().nonzero(), dtype=torch.long).t().to(device)
                    ...
                    if self.labels.max() > 1:
                        ...
                        try:
                            AUCROC = roc_auc_score(
                                self.labels[idx_test].detach().cpu().numpy(),
                                logits[idx_test].detach().cpu().numpy(),
                            )
                        except:
                            AUCROC = 'N/A'
                    (
    ...
    def predict_(self, idx_test):
        ...
        if self.labels.max() > 1:
            ...
            try:
                AUCROC = roc_auc_score(
                    self.labels[idx_test].detach().cpu().numpy(), logits[idx_test].detach().cpu().numpy()
                )
            except:
                AUCROC = 'N/A'
        (
    ...
    def predict_sens_group(self, output, idx_test):
        ...
        result = []
        for sens in [0, 1]:
            #print("device of labels:{}, pred:{}, sens:{}".format(self.labels.device, pred.device, self.sens.device))
            F1 = f1_score(
                ...
                .cpu()
                .numpy(),
                pred[self.sens[idx_test].detach().cpu().numpy() == sens].detach().cpu().numpy(),
                average="micro",
            )
            ACC = accuracy_score(
                self.labels[idx_test][
