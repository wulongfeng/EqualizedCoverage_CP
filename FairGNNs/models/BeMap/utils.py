...
def load_data(dataset_name, data_seed, sens_attr_idx, sens_number=500):
    print("parameter:{}, {}, {}, {}".format(dataset_name, data_seed, sens_attr_idx, sens_number))
    ...
    edges = data.edge_index.numpy()  # Convert from tensor if originally a tensor
    adj_coo = sp.coo_matrix((np.ones(edges.shape[1]), (edges[0], edges[1])), shape=(labels.shape[0], labels.shape[0]))
    adj_dense = adj_coo.toarray()  # 'toarray' converts sparse to dense numpy array
    adj_dense = np.maximum(adj_dense, adj_dense.T)
    # Add self-loops to the adjacency matrix
    adj = adj_dense + np.eye(adj_dense.shape[0])
...
def load_pokec(dataset, sens_attr, predict_attr,
               # path="../dataset/pokec/",
               path='../dataset/pokec',
               label_number=1000, sens_number=500, seed=19, test_idx=False):
    """Load data"""
...
def accuracy_logit(output, labels):
    output = output.squeeze()
    preds = (output > 0).type_as(labels)
    # _, preds = torch.max(output, dim=-1)
    correct = preds.eq(labels).double()
...
def load_credit(dataset, sens_attr="Age", predict_attr="NoDefaultNextMonth",
                path="../dataset/credit", label_number=1000, sens_number=500):
    print('Loading {} dataset from {}'.format(dataset, path))
    idx_features_labels = pd.read_csv(os.path.join(path, "{}.csv".format(dataset)))
    header = list(idx_features_labels.columns)
    header.remove(predict_attr)
...
def load_bail(dataset, sens_attr="WHITE", predict_attr="RECID",
              path="../dataset/bail", label_number=1000, sens_number=500):
    print('Loading {} dataset from {}'.format(dataset, path))
    idx_features_labels = pd.read_csv(os.path.join(path, "{}.csv".format(dataset)))
    header = list(idx_features_labels.columns)
    header.remove(predict_attr)
...
def load_pokec_emb(dataset, sens_attr, predict_attr,
                   # path="../dataset/pokec/",
                   path='../../FairGNN/dataset/pokec',
                   label_number=1000, sens_number=500, seed=19, test_idx=False):
    print('Loading {} dataset from {}'.format(dataset, path))
