...
class BeMap_GCN(BeMap):
    def __init__(self, nfeat, nhid, nclass, dropout, graph, attrs, beta=0.25, lam=0.5, save_num=4,
                use_dropout=True, layer_num=2, use_cuda=False, norm='both'):
    ...
    def forward(self, x, epoch):
        #print("input x({}):{}".format(x.shape, x))
        ...
        #print("x({}) after first gcn and relu:{}".format(x.shape, x))
        if self.use_dropout:
            x = self.dropout(x)
        #    print("x({}) after dropout:{}".format(x.shape, x))
        x = self.gc2(sg, x)
        #print("x({}) after second gcn layer:{}".format(x.shape, x))
        x = self.fc(x)
        #print("x({}) after the last fully connected layer:{}".format(x.shape, x))
        return x
