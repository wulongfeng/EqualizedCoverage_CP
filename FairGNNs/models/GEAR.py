...
class CFDA(nn.Module):
    ...
    def train_model(self, X, adj, sen_idx, dataset, model_path='', lr=0.0001, weight_decay=1e-5):
        ...
        for epoch in range(500): #2000 --> 500
            for i in range(3): # 3
...
def compute_loss(model, subgraph, cf_subgraph_list, labels, idx_select, n, sim_coeff, hidden_size, batch_size, device):
    ...
    l1 = loss_fn(c1, labels[idx_select].to(device)) /2
...
def evaluate(model, data, subgraph, cf_subgraph_list, labels, sens, idx_select, n, sim_coeff, hidden_size, batch_size, device='cuda', type='all'):
    ...
    if type == 'easy':
        ...
        output_preds = output.argmax(dim=-1)

        try:
            auc_roc = roc_auc_score(labels.cpu().numpy()[idx_select], output.detach().cpu().numpy(), multi_class='ovr')
        except:
            auc_roc = 'nan'
        f1_s = f1_score(labels[idx_select].cpu().numpy(), output_preds.cpu().numpy(), average="micro")
        ...
        for si in range(len(cf_subgraph_list)):
            ...
            output_preds_cf = output_cf.argmax(dim=-1)
...
class GEAR(torch.nn.Module):
    def __init__(self, adj, features, labels, idx_train, idx_val, idx_test, sens, sens_idx, num_class=1, dataset_name='None', hidden_size=1024, proj_hidden=16, encoder_hidden_size=1024, encoder_base_model='gcn', experiment_type='train'):
        ...
        #("fc1:{}".format(self.fc1))
        #print("fc2:{}".format(self.fc2))
        #print("fc3:{}".format(self.fc3))
        #print("fc4:{}".format(self.fc4))
    ...
    def fit(self, dataset_name="None", epochs=500, lr=0.001, batch_size=100, weight_decay=1e-5, sim_coeff=0.6, encoder_name="None", device='cuda'):
        print("Name of dataset:{}".format(dataset_name))
        ...
        for epoch in range(epochs + 1):
            ...
            l3 = loss_fn(c1, labels[sample_idx].to(device)) /2
    ...
    def predict(self, calib_test_mask, calib_test_sensitive_mask, calib_test_no_sensitive_mask, n_calib, alpha, dataset_name="None", encoder_name="None", batch_size=100, sim_coeff=0.6):
    #def predict(self, dataset_name="None", encoder_name="None", batch_size=100, sim_coeff=0.6):
        print("predction of dataset:{}".format(dataset_name))
        results_all_exp = {}
        # ========= test all ===========
        ...
        idx_all_mask = torch.tensor([True] * n)
        emb_all = get_all_node_emb(self, idx_all_mask, self.subgraph, n, self.hidden_size, batch_size)

        '''output of the linear layer?'''
        #output = self.forwarding_predict(emb)
        output_all = self.forwarding_predict(emb_all)
        with open('saved_output/{}_gear_epoch_{}.pkl'.format(self.dataset, self.epochs), 'wb') as f:
            pickle.dump(output_all, f)

        output = output_all[self.idx_test]
        #output_preds = (output.squeeze() > 0).type_as(self.data.y)
        output_preds = output.argmax(dim=-1)
        output_logits = torch.nn.Softmax(dim = 1)(output)
        ...
        F1 = f1_score(labels[idx_test], output_preds.cpu().numpy(), average='micro')
        ACC = accuracy_score(labels[idx_test], output_preds.cpu().numpy(), )
        try:
            AUCROC = roc_auc_score(labels[idx_test], output_preds.cpu().numpy(), multi_class='ovr')
        except:
            AUCROC = 'nan'


        cov_all, ineff_all, cov_sen, ineff_sen, cov_nosen, ineff_nosen = pred_conformal_classification(output_all, self.labels, calib_test_mask, calib_test_sensitive_mask, calib_test_no_sensitive_mask, n_calib, alpha)
        return (
            ACC,
            AUCROC,
            F1,
        ...
        return ACC, AUCROC, F1, ACC_sens0, AUCROC_sens0, F1_sens0, ACC_sens1, AUCROC_sens1, F1_sens1, SP, E
        '''
    ...
    def predict_sens_group(self, output, idx_test):
        ...
        for sens in [0,1]:
            ...
            try:
                AUCROC=roc_auc_score(self.labels[idx_test][self.sens[idx_test]==sens].detach().cpu().numpy(), pred[self.sens[idx_test]==sens], multi_class='ovr')
            except:
                AUCROC='nan'
