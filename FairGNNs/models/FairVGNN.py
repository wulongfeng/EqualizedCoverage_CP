...
def evaluate_ged3(x, classifier, discriminator, generator, encoder, data, args):
    ...
    #pred_val = (output[data.val_mask].squeeze() > 0).type_as(data.y)
    #pred_test = (output[data.test_mask].squeeze() > 0).type_as(data.y)
    pred_val = output[data.val_mask].argmax(dim=-1)
    ...
    F1s['val'] = f1_score(data.y[data.val_mask].cpu().numpy(), pred_val.cpu().numpy(), average='micro')


    #loss_fn=torch.nn.BCELoss()
    #F1s['val_loss']=loss_fn(output[data.val_mask].squeeze().sigmoid(), torch.tensor(data.y[data.val_mask]).float()).item()

    F1s['test'] = f1_score(data.y[data.test_mask].cpu().numpy(), pred_test.cpu().numpy(), average='micro')

    F1s['test_sens0']= f1_score(data.y[data.test_mask][data.sens[data.test_mask]==0].cpu(
    ).numpy(), pred_test[data.sens[data.test_mask]==0].cpu().numpy(), average='micro')
    F1s['test_sens1']= f1_score(data.y[data.test_mask][data.sens[data.test_mask]==1].cpu(
    ).numpy(), pred_test[data.sens[data.test_mask]==1].cpu().numpy(), average='micro')

    logits = torch.nn.Softmax(dim = 1)(output)
    max_probs, _ = torch.max(logits, dim=1)
    try:
        auc_rocs['val'] = roc_auc_score(
            data.y[data.val_mask].cpu().numpy(), logits[data.val_mask].detach().cpu().numpy(), multi_class='ovr')
        auc_rocs['test'] = roc_auc_score(
            data.y[data.test_mask].cpu().numpy(), logits[data.test_mask].detach().cpu().numpy(), multi_class='ovr')
    except:
        auc_rocs['val'] = 'nan'
        auc_rocs['test'] = 'nan'
    ...
    try:
        auc_rocs['test_sens0'] = roc_auc_score(
            data.y[data.test_mask][data.sens[data.test_mask]==0].cpu().numpy(), logits[data.test_mask][data.sens[data.test_mask]==0].detach().cpu().numpy(), multi_class='ovr')
        auc_rocs['test_sens1'] = roc_auc_score(
            data.y[data.test_mask][data.sens[data.test_mask]==1].cpu().numpy(), logits[data.test_mask][data.sens[data.test_mask]==1].detach().cpu().numpy(), multi_class='ovr')
    except:
    ...
    return output, accs, auc_rocs, F1s, paritys, equalitys
    ...
    def run(self, data, args):
        ...
        for count in range(args.runs):
            ...
            for epoch in tqdm(range(0, args.epochs)):
                ...
                for epoch_c in range(0, args.c_epochs):
                    ...
                    if (args.f_mask == 'yes'):
                        ...
                        for k in range(args.K):
                            ...
                            #print("type of y:{}, and value:{}".format(data.y.dtype, data.y[data.train_mask]))
                            loss_c = torch.nn.CrossEntropyLoss()(output[data.train_mask], data.y[data.train_mask].to(torch.long).to(args.device))
                        ...
                        #loss_c = F.binary_cross_entropy_with_logits(
                        #    output[data.train_mask], data.y[data.train_mask].unsqueeze(1).to(args.device))
                        loss_c = torch.nn.CrossEntropyLoss()(output[data.train_mask], data.y[data.train_mask].to(args.device))
                ...
                output, accs, auc_rocs, F1s, tmp_parity, tmp_equality = evaluate_ged3(
                ...
                #print("auc type:{}, value:{}".format(type(auc_rocs['val']), auc_rocs['val']))
                #print("f1s type:{}, value:{}".format(type(F1s['val']), F1s['val']))
                #print("accs type:{}, value:{}".format(type(accs['val']), accs['val']))
                #print("args.alpha type:{}, value:{}".format(type(args.alpha), args.alpha))
                #print("tmp_parity type:{}, value:{}".format(type(tmp_parity['val']), tmp_parity['val']))
                #print("tmp_equality type:{}, value:{}".format(type(tmp_equality['val']), tmp_equality['val']))
        ...
        return best_output, acc, f1, auc_roc, parity, equality, test_acc_sens0, test_acc_sens1, test_auc_roc_sens0, test_auc_roc_sens1, test_f1_sens0, test_f1_sens1


    def fit(self,adj, feats, labels, idx_train, idx_val, idx_test, sens, sens_idx, runs=2, epochs=200, dataset='Cora', d_epochs=5, g_epochs=5,
        ...
        args.num_features, args.num_classes = data.x.shape[1], len(data.y.unique())
        ...
        print("train ratio shape:{}, value:{}".format(args.train_ratio.shape, args.train_ratio))
        ...
        self.args=args
        print('running')
        self.output, self.acc, self.f1, self.auc_roc, self.parity, self.equality, self.test_acc_sens0, self.test_acc_sens1, self.test_auc_roc_sens0, self.test_auc_roc_sens1, self.test_f1_sens0, self.test_f1_sens1 = self.run(data, args)

    def predict(self, labels, calib_test_mask, sensitive_mask, no_sensitive_mask, n_sample, alpha):

        output, acc, f1, auc_roc, parity, equality, test_acc_sens0, test_acc_sens1, test_auc_roc_sens0, test_auc_roc_sens1, test_f1_sens0, test_f1_sens1 = self.output, self.acc, self.f1, self.auc_roc, self.parity, self.equality, self.test_acc_sens0, self.test_acc_sens1, self.test_auc_roc_sens0, self.test_auc_roc_sens1, self.test_f1_sens0, self.test_f1_sens1
        with open('saved_output/{}_fairvgnn_epoch_{}.pkl'.format(self.dataset, self.epochs), 'wb') as f:
        ...
        cov_all, ineff_all, cov_sen, ineff_sen, cov_nosen, ineff_nosen = pred_conformal_classification(output, labels, calib_test_mask, sensitive_mask, no_sensitive_mask, n_sample, alpha)
