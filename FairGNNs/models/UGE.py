...
class UGE():
    def fit(self, adj, feats, labels, idx_train, sens, nclass, dataset='Cora', max_iter=50000, model='gcn', debias_method='uge-w', debias_attr='gender', reg_weight=0.5, loss='entropy',lr=0.01/20,
            weight_decay=5e-4, dim1=64, dim2=32, predictor='dot', seed=0, device=0, epochs=690):
        self.labels=labels
        ...
        self.lgreg = LogisticRegression(penalty='none', random_state=1, class_weight='balanced',solver='lbfgs', max_iter=self.max_iter).fit(
            self.embs[idx_train], labels[idx_train])


        self.lgreg_sens = LogisticRegression(random_state=0, class_weight='balanced', max_iter=5000).fit(
            self.embs[idx_train], self.sens[idx_train])
        '''
        self.Linear1 = torch.nn.Linear(self.embs.shape[-1], nclass)
        optimizer = torch.optim.Adam(list(self.Linear1.parameters()), lr=5e-3)
        ...
        try:
            AUCROC=roc_auc_score(self.labels[idx_train], logits, multi_class='ovr')
        except:
            AUCROC='N/A'
        print("Performance of training set, F1:{}, and Acc:{}, Aucroc:{}".format(F1, ACC, AUCROC))
    ...
    def predict(self, idx_test, labels, calib_test_mask, calib_test_sensitive_mask, calib_test_no_sensitive_mask, n, alpha, idx_val):
        ...
        pred_prob = self.lgreg.predict_proba(self.embs)
        with open('saved_output/{}_uge_epoch_{}.pkl'.format(self.dataset, self.max_iter), 'wb') as f:
        ...
        cov_all, ineff_all, cov_sen, ineff_sen, cov_nosen, ineff_nosen = prob_conformal_classification(pred_prob, labels, calib_test_mask, calib_test_sensitive_mask, calib_test_no_sensitive_mask, n, alpha)
        return (
            ACC,
            'N/A',
            F1,
