...
class FairWalk:
    ...
    def fit(
        ...
        self.Linear2 = torch.nn.Linear(32, nclass)

        optimizer = torch.optim.Adam(list(self.Linear1.parameters())+list(self.Linear2.parameters()), lr=1e-3)
        #optimizer = torch.optim.Adam(list(self.Linear1.parameters()), lr=1e-3)
        #loss_fn=torch.nn.BCELoss()
        loss_fn = torch.nn.CrossEntropyLoss()

        for i in range(self.epochs):
           #idx=np.random.choice(idx_train, size=50, replace=False)
           idx=idx_train
           optimizer.zero_grad()

           #loss=loss_fn(torch.nn.functional.sigmoid(self.Linear2(self.Linear1(torch.tensor(self.embs[idx]).float())).squeeze()), torch.tensor(self.labels[idx]).float())
           y_pred = self.Linear2(self.Linear1(torch.tensor(self.embs[idx]).float()))
           #y_pred = self.Linear1(torch.tensor(self.embs[idx]).float())
           loss = loss_fn(y_pred, torch.tensor(self.labels[idx]))
           #if i % 50 == 0:
               #print("labels:{}".format(self.labels[idx]))
               #print("loss:{}".format(loss))
           loss.backward()
           optimizer.step()

        #pred = (torch.nn.functional.sigmoid(self.Linear2(self.Linear1(torch.tensor(self.embs[idx_train]))))>0.5).float().squeeze()
        output = self.Linear2(self.Linear1(torch.tensor(self.embs[idx_train])))
        #output = self.Linear1(torch.tensor(self.embs[idx_train]))
    ...
    def classify(self, idx_test, idx_val):
        ...
        for iter in range(1):  # 200
            ...
            #print("y_prob({}):{}".format(y_prob.shape, y_prob))
    ...
    def predict(self, idx_test, labels, calib_test_mask, calib_test_sensitive_mask, calib_test_no_sensitive_mask, n, alpha, idx_val):
        ...
        #output = self.Linear2(self.Linear1(torch.tensor(self.embs[idx_test])))
        output = self.Linear2(self.Linear1(torch.tensor(self.embs)))
        #output = self.Linear1(torch.tensor(self.embs))
        # with open('saved_output/{}_fairwalk_epoch_{}.pkl'.format(self.dataset, self.epochs), 'wb') as f:
        #     pickle.dump(output, f)

        pred = output[idx_test].argmax(dim=-1)
        ...
        AUCROC = roc_auc_score(self.labels[idx_test], logits.detach().cpu().numpy(), multi_class='ovr')

        cov_all, ineff_all, cov_sen, ineff_sen, cov_nosen, ineff_nosen = pred_conformal_classification(output, labels, calib_test_mask, calib_test_sensitive_mask, calib_test_no_sensitive_mask, n, alpha)
        return (
            ACC,
            AUCROC,
            F1,
        ...
        (
            ...
            AUCROC_sens1,
            F1_sens1,
        ) = self.predict_sens_group(pred, self.labels[idx_test], idx_test)
