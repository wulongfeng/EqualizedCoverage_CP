...
class NIFTY(torch.nn.Module):
    def __init__(
        ...
        lr=0.001,
        weight_decay=1e-5,
        drop_edge_rate_1=0.15,
        drop_edge_rate_2=0.15,
        drop_feature_rate_1=0.15,
        drop_feature_rate_2=0.15,
        encoder="gcn",
        sim_coeff=0.5,
        device="cuda",
    ):
        ...
        #print("device of adj:{}".format(adj.device))
    ...
    def ssf_validation(self, x_1, edge_index_1, x_2, edge_index_2, y):
        ...
        '''
        l3 = (loss_fn(c1[self.idx_train], self.labels[self.idx_train].to(self.device)) / 2 )
        l4 = (loss_fn(c2[self.idx_train], self.labels[self.idx_train].to(self.device)) / 2 )
    ...
    def fit_GNN(self, epochs=300):
        ...
        for epoch in range(epochs + 1):
            ...
            #)
            cl_loss = loss_fn(c1[self.idx_train], self.labels[self.idx_train].to(self.device))
            ...
            #)
            val_loss = loss_fn(c_val[self.idx_val], self.labels[self.idx_val].to(self.device))
            # if epoch % 100 == 0:
    ...
    def fit(self, epochs=300):
        ...
        for epoch in range(epochs + 1):
            ...
            preds = output[self.idx_val].argmax(dim=-1)
            ...
            print("acc on val:{}".format(ACC))
            '''

            #if epoch % 100 == 0:
            #    print(f"[Train] Epoch {epoch}:train_s_loss: {(sim_loss/rep):.4f} | train_c_loss: {cl_loss:.4f} | val_s_loss: {val_s_loss:.4f} | val_c_loss: {val_c_loss:.4f} | val_acc: {ACC_val:.4f}")
            ...
            if (val_c_loss + val_s_loss) < best_loss:
                ...
                if not os.path.exists("data"):
                    os.makedirs("data")
                torch.save(self.state_dict(), f"data/weights_ssf_{self.encoder}.pt")

        emb = self.forward(self.val_x_1, self.val_edge_index_1)
        output = self.forwarding_predict(emb)
        preds = output[self.idx_train].argmax(dim=-1)
        ACC = accuracy_score(
            self.labels.cpu().numpy()[self.idx_train],
            preds.detach().cpu().numpy(),
        )
        print("acc on training set:{}".format(ACC))
    ...
    def predict_GNN(self):
        ...
        output = self.forwarding_predict(emb)
        output_preds = output[self.idx_test].argmax(dim=-1).detach().cpu().numpy()
        '''
        output_preds = (
            (output.squeeze() > 0)
        ...
        try:
            AUCROC = roc_auc_score(labels[idx_test], output_preds, multi_class='ovr')
        except:
            AUCROC = "N/A"
    ...
    def predict(self, calib_test_mask, calib_test_sensitive_mask, calib_test_no_sensitive_mask, n, alpha):

        self.load_state_dict(torch.load(f"data/weights_ssf_{self.encoder}.pt"))
        self.eval()
        emb = self.forward(
            self.features.to(self.device), self.edge_index.to(self.device)
        )
        output = self.forwarding_predict(emb)
        with open('saved_output/{}_nifty_epoch_{}.pkl'.format(self.dataset, self.epochs), 'wb') as f:
        ...
        cov_all, ineff_all, cov_sen, ineff_sen, cov_nosen, ineff_nosen = pred_conformal_classification(output, self.labels, calib_test_mask, calib_test_sensitive_mask, calib_test_no_sensitive_mask, n, alpha)
        return (
            ACC,
            AUCROC,
            F1,
...
def drop_feature(x, drop_prob, sens_idx, sens_flag=True):
    ...
    x = x.clone()
    #print("the shape of drop_mask:{}, x:{}".format(drop_mask.shape, x.shape))
    #print("x:{}".format(x))
    #print("sens_idx:{}".format(sens_idx))
    drop_mask[sens_idx] = False


    #x[:, drop_mask] += torch.ones(1).normal_(0, 1).to(x.device)
    x[:, drop_mask] += torch.ones(1).normal_(0, 1).cuda()
