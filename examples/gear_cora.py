...
for epo in [500, 550, 600, 650, 700, 750, 800, 850, 900, 950, 1000]:
    ...
    model = GEAR(adj, feats, labels, idx_train, idx_val, idx_test, sens, params["sens_attr_idx"], params["num_class"], params["dataset"]).cuda()
    model.fit(params["dataset"], epochs=epo)
    ...
    (
        ...
        cov_nosen,
        ineff_nosen
    ) = model.predict(calib_test_mask, sensitive_mask, no_sensitive_mask, n_sample, params['alpha'], params["dataset"])
